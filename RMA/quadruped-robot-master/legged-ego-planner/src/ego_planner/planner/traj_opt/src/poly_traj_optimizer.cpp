...
namespace ego_planner
{
      ...
      double &final_cost)
  {
    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        flag_dyn_infeas = sqrt(maxv_measure_) / max_vel_ > 1.05 || sqrt(maxa_measure_) / max_acc_ > 1.05 || sqrt(maxj_measure_) / max_jer_ > 1.05;
        ...
        else
        {
          ...
          {
            PRINTF_COND("Swarm clearance not satisfied, keep optimizing. iter=%d,time(ms)=%5.3f, wei_swarm_mod_=%f -> %f\n", iter_num_, time_ms, wei_swarm_mod_, wei_swarm_mod_ * 10);
            wei_swarm_mod_ *= 10;
          }
          if (flag_dyn_infeas)
          {
            PRINTF_COND("Dynamical infeasible, keep optimizing. iter=%d,time(ms)=%5.3f, wei_feas_mod_=%f -> %f\n", iter_num_, time_ms, wei_feas_mod_, wei_feas_mod_ * 10);
            wei_feas_mod_ *= 10;
          }
        }
      }
                                               ...
                                               double &costv)
  {
    ...
    double vpen = v_squnorm - max_vel_ * max_vel_;
    if (vpen > 0)
    {
                                               ...
                                               double &costa)
  {
    ...
    double apen = a_squnorm - max_acc_ * max_acc_;
    if (apen > 0)
    {
                                               ...
                                               double &costj)
  {
    ...
    double jpen = j_squnorm - max_jer_ * max_jer_;
    if (jpen > 0)
    {
      gradj = wei_feas_mod_ * 6 * jpen * jpen * j;
