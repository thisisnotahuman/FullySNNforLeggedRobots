...
ros::Publisher vis_vel_pub;

quadrotor_msgs::PositionCommand cmd;
geometry_msgs::PoseStamped vis_pos;
visualization_msgs::Marker vis_vel;
...
void publishcmd(Vector3d p, Vector3d v, Vector3d a, Vector3d j, double y, double yd)
{
  ...
  cmd.jerk.z = j(2);
  cmd.yaw = y;
  cmd.yaw_dot = y/3.1415926*360;
  pos_cmd_pub.publish(cmd);
  last_pos_ = p;

  vis_pos.header = cmd.header;
  vis_pos.pose.position.x = cmd.position.x;
  vis_pos.pose.position.y = cmd.position.y;
  vis_pos.pose.position.z = cmd.position.z;
  tf::Quaternion q = tf::createQuaternionFromYaw(cmd.yaw);
  geometry_msgs::Quaternion odom_quat;
  tf::quaternionTFToMsg(q, odom_quat);
  vis_pos.pose.orientation = odom_quat;
  vis_pos_pub.publish(vis_pos);

  vis_vel.ns = "vel";
  vis_vel.id = 0;
  vis_vel.header = cmd.header;
  vis_vel.pose.orientation.w = 1.0;
  ...
  pt.x = cmd.position.x + cmd.velocity.x;
  pt.y = cmd.position.y + cmd.velocity.y;
  pt.z = cmd.position.z + cmd.velocity.z;
  vis_vel.points.push_back(pt);
  vis_vel.scale.x = 0.06;
  vis_vel.scale.y = 0.15;
  vis_vel.scale.z = 0.15;
  vis_vel_pub.publish(vis_vel);
}

  void dogStateCallback(const std_msgs::Int64ConstPtr &msg)
  {
    if (msg->data==1)
    {
        prepare_to_go_ = true;
...
void cmdCallback(const ros::TimerEvent &e)
{
  ...
  if ((time_now - heartbeat_time_).toSec() > 0.5)
  {
    ...
    receive_traj_ = false;
    publishcmd(last_pos_, Vector3d::Zero(), Vector3d::Zero(), Vector3d::Zero(), last_yaw_, 0);
  }
  if( !prepare_to_go_)
    return;
  ...
  if (t_cur < traj_duration_ && t_cur >= 0.0)
  {
    ...
    // publish
    publishcmd(pos, vel, acc, jer, yaw_yawdot.first, yaw_yawdot.second);
#if FLIP_YAW_AT_END or TURN_YAW_TO_CENTER_AT_END
    finished = false;
#endif
  }
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber dog_state_sub = nh.subscribe("/dog_state", 10, dogStateCallback);

  pos_cmd_pub = nh.advertise<quadrotor_msgs::PositionCommand>("/position_cmd", 50);
  vis_pos_pub = nh.advertise<geometry_msgs::PoseStamped>("desired_position", 50);
  vis_vel_pub = nh.advertise<visualization_msgs::Marker>("desired_velocity", 50);
