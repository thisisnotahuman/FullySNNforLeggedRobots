...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    way_point_vis_pub_ = nh.advertise<visualization_msgs::Marker>("waypoint_vis",1);
      ...
      const bool flag_randomPolyTraj, const bool touch_goal)
  {
    ...
    if (poly_traj_opt_->finelyCheckAndSetConstraintPoints(segments, initMJO, true) == PolyTrajOptimizer::CHK_RET::ERR)
    {
      return false;
    }
  ...
  bool EGOPlannerManager::setLocalTrajFromOpt(const poly_traj::MinJerkOpt &opt, const bool touch_goal)
  {
    ...
    bool ret = poly_traj_opt_->computePointsToCheck(traj, ConstraintPoints::two_thirds_id(cps, touch_goal), pts_to_check);
  ...
  void EGOPlannerManager::visSamplePoints(vector<Eigen::Vector3d> sample_points)
  {
      ...
      for(int i = 0; i < sample_points.size(); i++)
      {
          ...
          pt.x = sample_points[i](0);
          pt.y = sample_points[i](1);
          pt.z = sample_points[i](2); 
          points.points.push_back(pt);   
      }
      way_point_vis_pub_.publish(points);
  }
