...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    nh.param("fsm/distance_miss", distance_miss, 0.0);
    ...
    dog_state_sub_ = nh.subscribe("/dog_state", 1, &EGOReplanFSM::dogStateCallback, this);
    aim_sub_ = nh.subscribe("/object_tracking/aim/pose", 1, &EGOReplanFSM::aimCallback, this);
    ...
    aim_world_pub_ = nh.advertise<nav_msgs::Odometry>("/aim_world", 10);
    aim_body_pub_ = nh.advertise<nav_msgs::Odometry>("/aim_body", 10);
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    {
      ROS_INFO("SEQUENTIAL_START.");
      if (planner_manager_->pp_.drone_id <= 0 || (planner_manager_->pp_.drone_id >= 1 && have_recv_pre_agent_))
      {
        bool success = false;
        if (prepare_to_go_){
           std::cout << "ready prepare  to go"<< std::endl;
           success = planFromGlobalTraj(10); // zx-todo
        }
        if (success)
        {
        ...
        {
          std::cout << " not ready prepare  to go"<< std::endl;
    ...
    case GEN_NEW_TRAJ:
    {
      std::cout << "you miss me baby" << std::endl;
    ...
    case REPLAN_TRAJ:
    {
      ...
      plan_success = planFromLocalTraj(1);
    ...
    case EXEC_TRAJ:
    {
      ...
      if (modifyInCollisionFinalGoal()) // case 1: find that current goal is in obstacles
      {
        // pass
      }
      else if ((target_type_ == TARGET_TYPE::PRESET_TARGET) &&
  ...
  bool EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    // one_pt_wps = planner_manager_->calculateUTurnWaypoints(odom_pos_, odom_yaw_, next_wp);
    one_pt_wps.push_back(next_wp);
    // if (front_waypoints_.size() != 0)
    // {
    //   wps_.insert(std::next(wps_.begin(), wpt_id_ + 1), front_waypoints_.begin(), front_waypoints_.end());
    //   next_wp = wps_[wpt_id_];
    //   wpt_id ++ ;
    // }
    // odom_vel_(0) = 0.5 * cos(odom_yaw_);
    // odom_vel_(1) = 0.5* sin(odom_yaw_);
    // odom_vel_(2) = 0;
  ...
  void EGOReplanFSM::waypointCallback(const geometry_msgs::PoseStampedConstPtr &msg)
  {
    // if (msg->pose.position.z < -0.1)
    //   return;
    if(start_to_trace_ == false){
      start_to_trace_ = true;
    }
    else
    {
      have_target_ = false;
      start_to_trace_ = false;
    }
    // ROS_INFO("Received goal: %f, %f, %d", msg->pose.position.x, msg->pose.position.y, 0);
  ...
  void EGOReplanFSM::aimCallback(const nav_msgs::OdometryConstPtr &msg)
  {

     tf::StampedTransform transform;
    try{
      listener.lookupTransform("world", "target_link",  
    ...
    if (!start_to_trace_){
      ROS_INFO("Not ready to trace.");
      return;
    }
    // aim_body_pub_.publish(*msg);

    Eigen::Vector3d end_wp( x, y, 0.0);
    
    if(have_odom_)
    {
      //  end_wp(0) = odom_pos_(0) + end_wp(0) * cos(-odom_yaw_) + end_wp(1)  * sin(-odom_yaw_);
      //  end_wp(1) = odom_pos_(1) +  end_wp(0)  * -sin(-odom_yaw_) + end_wp(1)  * cos(-odom_yaw_);
       ROS_INFO("Received aim: %f, %f, %f", end_wp(0), end_wp(1), odom_yaw_);
       nav_msgs::Odometry aim_world;
       aim_world.pose.pose.position.x = end_wp(0);
       aim_world.pose.pose.position.y = end_wp(1);
       aim_world.pose.pose.position.z = 0;
       aim_world.header.frame_id = "world";
       aim_world_pub_.publish(aim_world);
       flagAimtrigger = true;
      if (planNextWaypoint(end_wp))
      {
        have_trigger_ = true;
  ...
  void EGOReplanFSM::readUTurnWpsAndPlan()
  {
    if (front_waypoints_.size() <= 0)
    {
      ROS_ERROR("Wrong front waypoint_num_ = %d", front_waypoints_.size());
      return;
    }else
    {
      ROS_INFO("waypoint_num_ = %d", front_waypoints_.size());
    }
    waypoint_num_ = front_waypoints_.size();
    wps_.resize(front_waypoints_.size());
    for (int i = 0; i < waypoint_num_; i++)
    {
      wps_[i](0) = front_waypoints_[i][0];
      wps_[i](1) = front_waypoints_[i][1];
      wps_[i](2) = 0;
    }

    for (size_t i = 0; i < front_waypoints_.size(); i++)
    {
      visualization_->displayGoalPoint(wps_[i], Eigen::Vector4d(0, 0.5, 0.5, 1), 0.1, i);
      ros::Duration(0.001).sleep();
    }
  ...
  void EGOReplanFSM::dogStateCallback(const std_msgs::Int64ConstPtr &msg)
  {
    if (msg->data==1)
    {
        prepare_to_go_ = true;
