...
class Covariance(nn.Module):
    ...
    def __init__(self,
        ...
        mask = torch.zeros((self.rank, self.param_rank), device=self.device)
        c = 0
    ...
    def _set_params(self, method: str, init_diag_multi: float):
        ...
        if method == 'log_cholesky':
            self.method = method
            self.cholesky_log_diag = nn.Parameter(.1 * torch.randn(self.param_rank, device=self.device) + math.log(init_diag_multi))
            self.cholesky_off_diag = nn.Parameter(.1 * torch.randn(num_off_diag(self.param_rank), device=self.device))
            ...
            self.lr_mat = nn.Parameter(data=.01 * torch.randn(self.param_rank, low_rank, device=self.device), device=self.device)
            self.log_std_devs = nn.Parameter(data=.1 * torch.randn(self.param_rank, device=self.device) + math.log(init_diag_multi), device=self.device)
        elif method == 'user_fixed':
            self.method = 'user_fixed'
            self.cov = torch.eye(self.param_rank, device=self.device) * init_diag_multi
        else:
