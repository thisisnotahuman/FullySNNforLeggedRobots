...
class StateSpaceModel(nn.Module):
    ...
    def __init__(self,
        ...
        self._scale_by_measure_var = False # True
    ...
    def _validate(processes: Sequence[Process], measures: Sequence[str]):
        ...
        for p in processes:
            ...
            if p.measure:
                if isinstance(p.measure, list):
                    for item in p.measure:
    ...
    def forward(self,
        ...
        preds, updates, R, H = self._script_forward(
            ...
            kwargs_per_process=self._parse_design_kwargs(  # 0.022 ms
                input,
    ...
    def _prepare_initial_state(self,
        ...
        # print(init_cov.shape)
        # print("init_cov = ", init_cov)
        ...
        # print("measure_scaling = ", measure_scaling)
        # print(init_cov)
    ...
    def _script_forward(self,
        ...
        """
        assert n_step > 0
        # print("out_timesteps = ", out_timesteps)
        ...
        # this func cost 1.0ms
        if getattr(self, "predict_kwargs") is None:
            ...
            predict_kwargs, update_kwargs = self.predict_kwargs, self.update_kwargs
        ...
        preds = [meanus[t] for t in range(out_timesteps)], [covus[t] for t in range(out_timesteps)]
        forcasting = False
        if forcasting:
            # idx: Dict[int, int] = {}
    ...
    def _get_measure_scaling(self) -> Tensor:
        ...
        if self._scale_by_measure_var:
            ...
            # print("self.state_rank = ", self.state_rank)
