...
class KalmanStep(StateSpaceStep):
    ...
    def _kalman_gain(cov: Tensor, H: Tensor, R: Tensor) -> Tensor:
        Ht = H.permute(0, 2, 1)  # (1, 3, 3)
        covs_measured = cov @ Ht
        # Hcov = torch.matmul(H, cov)
        # print(Hcov.shape) # (N, 3, 3)
        system_covariance = torch.baddbmm(R, H@cov, Ht.repeat(cov.size(0),1,1))
...
class KalmanFilter(StateSpaceModel):
    ...
    def __init__(self,
        ...
        for process in processes:
            process = process.to(self.device)
        initial_covariance = Covariance.from_processes(processes, cov_type='initial')

        if process_covariance is None:
            process_covariance = Covariance.from_processes(processes, cov_type='process',  method='user_fixed', init_diag_multi=0.1, device=self.device)

        if measure_covariance is None:
            measure_covariance = Covariance.from_measures(measures, method='user_fixed', init_diag_multi=0.1, device=self.device)
